...
Uso:
    ...
    pix = nm_svg_pixmap("home", color="#1C2218", size=24)
    label.setPixmap(pix)
"""
...
ICON_BODIES: dict[str, str] = {
    ...
    "timer": '<circle cx="12" cy="13" r="8"/><path d="M12 13V8M9 2h6M17 5l2 2"/>',
    "bell": '<path d="M6 9a6 6 0 0 1 12 0c0 6 2 7 2 7H4s2-1 2-7z"/>'
    '<path d="M10 20a2 2 0 0 0 4 0"/>',
    "user": '<circle cx="12" cy="8" r="3.5"/><path d="M4 21c0-3.5 3-6 8-6s8 2.5 8 6"/>',
    # Tuerca/engranaje real (feather "settings"): el glifo anterior (círculo +
    # rayos) parecía un sol y se confundía con el toggle de tema — feedback
    # owner v1.0.
    ...
    "medicine": '<rect x="3" y="9" width="18" height="6" rx="3"/><path d="M12 9v6"/>',
    ...
    "run": '<circle cx="13" cy="4" r="2"/><path d="M10 22l3-7 3 7M7 12l4-2 4 2-2 3h-4l-2-3z"/>',
    # ── Acciones ──────────────────────────────────────────────────────────
    ...
    "edit": '<path d="M3 21h4l11-11-4-4L3 17v4z"/><path d="M14 6l4 4"/>',
    ...
    "gem": '<path d="M6 3h12l3 6-9 12L3 9z"/><path d="M3 9h18M6 3l3 6L12 21M18 3l-3 6L12 21"/>',
    # ── Semantic / status ────────────────────────────────────────────────
    "warning": '<path d="M12 3l10 18H2z"/><path d="M12 10v5M12 18.5h.01"/>',
    ...
    "handshake": '<path d="M7 12l-3-3 4-4 3 3M17 12l3-3-4-4-3 3"/><path d="M7 12l5 5 5-5"/>',
    ...
    # Iconografía adicional requerida por el handoff visual.
    "shield": '<path d="M12 2l8 3v6c0 5-3.5 9-8 11-4.5-2-8-6-8-11V5l8-3z"/>',
    "sos": '<circle cx="12" cy="12" r="9"/>'
    '<path d="M9 9h-1.5c-.83 0-1.5.67-1.5 1.5v0c0 .83.67 1.5 1.5 1.5h1c.83 0 1.5.67 1.5 1.5v0c0 .83-.67 1.5-1.5 1.5H6"/>'
    '<circle cx="12" cy="12" r="2"/>'
    '<path d="M15 9h1.5c.83 0 1.5.67 1.5 1.5v3c0 .83-.67 1.5-1.5 1.5H15c-.83 0-1.5-.67-1.5-1.5v-3c0-.83.67-1.5 1.5-1.5z"/>',
    "library": '<rect x="4" y="3" width="4" height="18" rx="1"/>'
    '<rect x="10" y="3" width="4" height="18" rx="1"/>'
    '<path d="M17 4l4 1-4 16-1-4z"/>',
    "settings": '<circle cx="12" cy="12" r="3"/>'
    '<path d="M12 2v2M12 20v2M2 12h2M20 12h2M4.5 4.5l1.5 1.5M18 18l1.5 1.5M4.5 19.5l1.5-1.5M18 6l1.5-1.5"/>',
    "mind": '<path d="M7 4a3 3 0 0 0-3 3v3c0 1.5 1 2.5 1 4v3a3 3 0 0 0 3 3"/>'
    '<path d="M17 4a3 3 0 0 1 3 3v3c0 1.5-1 2.5-1 4v3a3 3 0 0 1-3 3"/>'
    '<path d="M8 9c1.5 0 3 1 3 3M16 9c-1.5 0-3 1-3 3M11 12v8M13 12v8"/>',
}
...
def nm_svg_pixmap(
    name: str, color: str = "#1C2218", size: int = 24, stroke_width: float | None = None
) -> QPixmap | None:
...
def _mood_emoji_svg(level: int, glow: bool, is_dark: bool) -> str:
    ...
    if glow:
        ...
        for i, r in enumerate((46, 49, 52)):
            ...
            parts.append(f'<circle cx="50" cy="50" r="{r}" fill="{color_glow}" opacity="{a:.3f}"/>')

    # 2. Círculo de cara (solo contorno)
    parts.append(
        f'<circle cx="50" cy="50" r="40" fill="none" stroke="{color_to}" stroke-width="{stroke}"/>'
    )
    ...
    if lv >= 7:
        ...
        parts.append(
            f'<circle cx="28" cy="58" r="3.5" fill="{color_from}" opacity="{blush_op:.2f}"/>'
        )
        parts.append(
            f'<circle cx="72" cy="58" r="3.5" fill="{color_from}" opacity="{blush_op:.2f}"/>'
        )
...
def nm_mood_pixmap(
    level: int, size: int = 64, glow: bool = True, is_dark: bool = False
) -> QPixmap | None:
