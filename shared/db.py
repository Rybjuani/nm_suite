...
@contextmanager
def conexion():
    """Conexión gestionada: commit al salir sin error, rollback ante excepción y
    ``close()`` SIEMPRE (incluido el camino de error).

    Evita el footgun de ``conn = obtener_conexion(); ... ; except: return`` que
    deja una transacción de escritura abierta y bloquea la base ("database is
    locked") para el resto de la sesión cuando un INSERT/UPDATE falla.
    """
...
def inicializar_tablas():
    ...
    try:
        ...
        pass
    # Registro pos/neg separado (v1.0 ronda 2): la valencia y la intensidad
    # CRUDA se persisten explícitas — el puntaje (bienestar 1-10) se conserva
    # como índice combinado, pero métricas y gráficos separan positivo de
    # negativo para no perder registros reales ("tristeza 10" NO es "ánimo 1").
    for _col, _typedef in (
        ("valencia", "TEXT DEFAULT ''"),
        ("intensidad", "INTEGER DEFAULT NULL"),
    ):
        try:
            conn.execute(f"ALTER TABLE termometro ADD COLUMN {_col} {_typedef}")
            conn.commit()
        except Exception:
            pass
    ...
    # tartamudea al cambiar de pestaña. IF NOT EXISTS: idempotente.
    for _idx_sql in (
        "CREATE INDEX IF NOT EXISTS idx_termometro_fecha ON termometro (fecha)",
        "CREATE INDEX IF NOT EXISTS idx_respiracion_fecha ON respiracion (fecha)",
        "CREATE INDEX IF NOT EXISTS idx_pensamientos_fecha ON pensamientos (fecha)",
        "CREATE INDEX IF NOT EXISTS idx_checklist_completadas_fecha "
        "ON checklist_completadas (fecha)",
        ...
        try:
            ...
            _log.warning("Índice de rendimiento no creado: %s", _idx_sql)
    conn.close()
...
def _migrar_tcc_templates_cache_schema(conn):
    cols = {
        row["name"] for row in conn.execute("PRAGMA table_info(tcc_templates_cache)").fetchall()
    }
    for col, typedef in (
        ("steps", "TEXT NOT NULL DEFAULT '[]'"),
        ("emotions", "TEXT NOT NULL DEFAULT '[]'"),
        ...
        if col not in cols:
            try:
                conn.execute(f"ALTER TABLE tcc_templates_cache ADD COLUMN {col} {typedef}")
                conn.commit()
            except Exception:
                pass
...
def _migrar_activacion_actividades_animo(conn):
    ...
    if "energia_min" in sql:
        try:
            conn.execute(
                "ALTER TABLE activacion_actividades RENAME COLUMN energia_min TO animo_min"
            )
            conn.commit()
        except Exception:
            pass
    if "energia_max" in sql:
        try:
            conn.execute(
                "ALTER TABLE activacion_actividades RENAME COLUMN energia_max TO animo_max"
            )
            conn.commit()
        except Exception:
            pass
