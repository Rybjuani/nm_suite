...
def replace_scopes(scopes, rows: Iterable[dict]) -> int:
    """Reemplaza el cache local de los `scopes` dados con EXACTAMENTE `rows`.

    Borra primero todas las filas locales de esos scopes y luego inserta las
    remotas. Sin esto, un override que el profesional BORRÓ en el Hub
    (Personalización → "Restablecer por defecto") seguía vivo en el cache local
    y la Suite nunca volvía al texto por defecto: `cache_rows` solo hacía
    INSERT OR REPLACE y nunca eliminaba claves ausentes. Reconciliación
    completa = el cache espeja el remoto.
    """
    scopes = tuple(scopes)
    try:
        conn = obtener_conexion()
    except Exception as exc:
        _log.warning("replace_scopes: no se pudo abrir DB: %s", exc)
        return 0
    try:
        for scope in scopes:
            conn.execute("DELETE FROM remote_config_cache WHERE scope = ?", (str(scope),))
        conn.commit()
    except Exception as exc:
        _log.warning("replace_scopes: error limpiando scopes: %s", exc)
    finally:
        try:
            conn.close()
        except Exception:
...
def refresh_from_supabase(patient_id: Optional[str] = None) -> bool:
    ...
    try:
        ...
        try:
            ...
            _log.debug("refresh_from_supabase: no se pudo configurar sesion auth (%s)", exc)
    except Exception as exc:
    ...
    try:
        res = sb.table("hub_config").select("scope, key, value").in_("scope", scopes).execute()
    except Exception as exc:
