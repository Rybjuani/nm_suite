...
## Decision guardrails (Fase 2 amend)

The classifier runs through a final coherence pass (`_enforce_decision_guardrails`)
before producing `agent_package.json`. These guardrails implement the owner audit
rules and cannot be bypassed:

| Rule | Trigger | Action |
|------|---------|--------|
| A — fidelity integrity | `FIDELITY_REPORT.json` is missing, empty, or has 0 comparisons | Stamps `fidelity_available=false` on every `agent_package.json`; metric numbers in report are not real signal |
| B — fuzzy ceiling | `text_evidence.fuzzy_ratio_worst >= 95` | Strips `TEXT_MISMATCH_PROBABLE`; downgrades text-only `FIX_PRODUCT_REVIEW` to `NEEDS_HUMAN_REVIEW` |
| B — empty diff | `text_evidence.diff_summary == "No significant OCR difference"` | Same as above |
| C — coherence | `decision_reason` says "OCR matches well" but any bbox has `fuzzy < 95` | Replaces `decision_reason` with evidence-derived text |
| D — confidence-floor | `confidence == low` | Forces `decision = NEEDS_HUMAN_REVIEW` (hard rule, no exceptions) |
| D — strong-needs-high | `decision == FIX_PRODUCT_STRONG` and `confidence != high` | Downgrades to `FIX_PRODUCT_REVIEW` (medium) or `NEEDS_HUMAN_REVIEW` (low) |
| E — actionable-hint | `decision in {FIX_PRODUCT_*, ...}` but `what_to_check_first` is generic | Downgrades to `NEEDS_HUMAN_REVIEW` |
| F — bbox dominance | `largest_bbox.area_ratio > LARGEST_BBOX_GUARDRAIL (0.35)` | Strips `TEXT_MISMATCH_PROBABLE` and `COLOR_MISMATCH` from labels; forces `confidence=low`; `decision=NEEDS_HUMAN_REVIEW` with reason mentioning `largest_bbox_area_ratio` |

The list of forbidden generic phrases (`FORBIDDEN_GENERIC_PHRASES`) covers:
...
`what_to_check_first` is now built concretely from the bbox that drives
the decision (highest `color_delta` for color-driven `FIX_PRODUCT_*`,
lowest `fuzzy_ratio_worst` for text-driven). It always mentions at least:
the OCR pair (if any), the color delta with mockup/real hex (if any),
the bbox `area_ratio`, and the `suspected_module`.

## diff_fidelity integration

`diff_fidelity.py` is a file-name-driven tool: it expects PNGs shaped as
`{app}-{view}-{theme}-{WxH}.png` in a flat directory. The canonical
populated directory for V3 is **`qa/_mockup_targets_normalized/`** (already
gitignored, 86 PNGs, generated by `normalize_mockup_reference.py`).

The directory `qa/mockup_reference_normalized/` (where `manifest.json` lives)
uses a *different* convention (`light/{view}.png` / `dark/{view}.png`) that
matches the Fase 1 pairing logic in V3 but **does not** match `diff_fidelity.py`.
Running `diff_fidelity.py --target-dir qa/mockup_reference_normalized` will
report `Targets considered: 0` because the regex `(suite|hub)-(.+)-(light|dark)-(\d+x\d+)\.png`
never matches the flat-named PNGs in `light/` and `dark/`.

Correct command:

```powershell
.\.venv\Scripts\python.exe qa\diff_fidelity.py `
  --target-dir qa\_mockup_targets_normalized `
  --actual-dir qa\_captures_v8 `
  --out-dir qa\_fidelity_current
# Expected output:
#   Targets considered: 86
#   Compared:           68   (18 = detalle-resumen-ia x2 in light/dark with
#                                state_id="0" not in normalized manifest — pre-existing)
#   Passed/Partial/Failures are based on SSIM/MAD/Changed against the real gate.
```

V3 `doctor` validates the report and emits a clear `[WARN]` if `FIDELITY_REPORT.json`
is missing, empty, or has 0 comparisons; every `agent_package.json` then carries
`fidelity_available=false` so downstream consumers can distinguish real metrics
from absent ones.
