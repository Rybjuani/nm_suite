...
- **Fix:** `_empty_hint_label` ahora devuelve un bloque **centrado** (h+v) con presencia vertical (`minimumHeight 220` + stretches arriba/abajo, texto `AlignCenter`). Se centra incluso bajo el `list_lay` con `AlignTop`. Un solo helper → arregla los **5 estados vacíos**:
...
| Vista | Resultado |
|---|---|
| `hub-detalle-plan-{dark,light}` (Recordatorios) | revisado — vacíos centrados, acciones diferenciadas |
| `hub-detalle-plan-activacion-{dark,light}` | revisado — "Sin actividades…" centrado |
| `hub-detalle-plan-rutina-{dark,light}` | revisado — "Sin tareas asignadas…" centrado |
| `hub-detalle-plan-timer-{dark,light}` | revisado — "…no tiene temporizadores…" centrado |
