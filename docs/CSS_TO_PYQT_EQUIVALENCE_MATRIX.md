...
| Selector / patrón | Fuente | PyQt actual / propuesto | No-equiv | Archivos | Tests/probes | Keys |
|---|---|---|---|---|---|---|
| `.modal-bg{rgba dim + backdrop-filter:blur(3px)}` | L357-358 | `NMDialog` / `_PracticeModalScrim` backdrop | requiere `window_overlay` con pantalla trasera real + blur/dim/backdrop equivalente; dim sin blur no es escape general → MISMATCH#17 | `components/dialogs.py:59`, `app/modules/dbt_qt.py` | `tools/qa/audit_modal_backdrop_blur.py` + exact key | dbt practice, ia modal |
...
| resumen IA (window overlay 960×600; panel 720×462) | (csv) | `NMDialog` / panel IA | `panel_crop` 720×462 es parcial; cierre requiere `window_overlay` + `back_screen_key=hub:detalle@*` | `hub/ia_asistente.py`, `hub/pacientes_qt.py` | `--key hub:detalle-resumen-ia-0@*` + auditor modal/backdrop | hub resumen ia |
