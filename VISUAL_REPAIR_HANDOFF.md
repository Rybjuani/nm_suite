...
- [x] `suite:recuperar-acceso@light` - severity=high; findings=raw_pixel_delta,state_or_recipe_suspect,qa_missed_raw_or_layout; changed=0.3327; odiff=5.47; bbox=12; panel=`reports\qa\layered_visual_compare_fresh\panels\suite_recuperar-acceso_light.png`.
  - Closure evidence (2026-06-28): fix commit `b0286be`; capture command `.\.venv\Scripts\python.exe qa\capture_v8.py --app suite --view recuperar-acceso --theme light --out-dir qa\_captures_v8 --no-clean`; report `reports\qa\layered_visual_compare_item\LAYERED_VISUAL_REPORT.json`; `REPORT_EVIDENCE_VALID: YES`; exact key `suite:recuperar-acceso@light` status `PASS`; `HANDOFF_CLOSURE_ALLOWED: NO` only because `REPORT_SCOPE: PARTIAL`; manual panel review confirms the recovery prompt, focused email field, consent block, feedback line, titlebar, and footer buttons match the canonical side-by-side.
