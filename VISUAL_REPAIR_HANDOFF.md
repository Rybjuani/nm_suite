...
- [x] `hub:detalle-plan-timer@dark` - severity=high; findings=raw_pixel_delta,layout_drift,qa_missed_raw_or_layout; changed=0.45219; odiff=6.04; bbox=143; panel=`reports\qa\layered_visual_compare_fresh\panels\hub_detalle-plan-timer_dark.png`.
  - Closure evidence (2026-06-30, fix commit `a0bb41b`): Hub detail plan now paints the canonical full screen surface, restores the `.screen` 24px padding, keeps the bottom window edge visible for bbox, and localizes the timer form/list geometry to the canonical `.hub-grid` rhythm. Exact dark capture+compare via `.\qa\run_visual_item.ps1 -App hub -View detalle-plan-timer -Theme dark -OutDir reports\qa\codex_detalle_plan_timer_dark_final`; report `reports\qa\codex_detalle_plan_timer_dark_final\LAYERED_VISUAL_REPORT.json` (`REPORT_EVIDENCE_VALID: YES`, `REPORT_SCOPE: PARTIAL`, `HANDOFF_CLOSURE_ALLOWED: NO` only because `partial_scope`); exact key `hub:detalle-plan-timer@dark` status `PASS` (`changed_pixel_ratio=0.09622`, `mean_abs_diff=0.03242`, `windowed_ssim=0.75092`, `max_bbox_delta_px=16`, `odiff=2.62`, `suspicious_perfect_match=false`). Anti-fraud CLEAN in wrapper. Anchor `hub:detalle-plan-timer@light` remains `PASS` via `.\qa\run_visual_item.ps1 -App hub -View detalle-plan-timer -Theme light -OutDir reports\qa\codex_detalle_plan_timer_light_anchor_final`; report `reports\qa\codex_detalle_plan_timer_light_anchor_final\LAYERED_VISUAL_REPORT.json` (`changed_pixel_ratio=0.09424`, `mean_abs_diff=0.03220`, `windowed_ssim=0.76453`, `max_bbox_delta_px=15`, `odiff=2.53`, `suspicious_perfect_match=false`). Cheap-agent WIP was validated only with the visual harness in an isolated temp worktree and rejected: `hub:detalle-plan-timer@dark` and `@light` both stayed `FAIL` with valid reports. Manual panel review confirms a real Qt render, no canonical/reference overlay or blit; residual diff is bounded to text/edge AA and normal Qt button/text rasterization.
