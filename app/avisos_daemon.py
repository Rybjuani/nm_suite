...
try:
    ...
    import winotify as _winotify_mod

    _WINOTIFY_OK = True

    def _run_ps_sin_consola(*, file="", command=""):
        """Reemplazo de winotify._run_ps con CREATE_NO_WINDOW.

        La versión de la librería solo usa STARTUPINFO/SW_HIDE: en un exe
        windowed (PyInstaller --noconsole) PowerShell ALOCA una consola nueva
        y en algunos Win10 el host se pinta un instante antes de ocultarse —
        la "mini ventana negra titilante" que reportó el owner al vencer un
        recordatorio. CREATE_NO_WINDOW evita la consola por completo.
        """
        import subprocess
    ...
    _winotify_mod._run_ps = _run_ps_sin_consola
except ImportError:
    _WINOTIFY_OK = False
...
def _reactivar_medianoche():
    ...
    try:
        with conexion() as conn:
            rows = conn.execute("SELECT id, dias FROM recordatorios WHERE activo = 0").fetchall()
            for row in rows:
                ...
                if dia_hoy in dias_activos:
                    conn.execute("UPDATE recordatorios SET activo = 1 WHERE id = ?", (rid,))
    except Exception:
...
def _notificar(mensaje: str, hora: str):
    ...
    try:
        ...
        _log.debug("plyer falló — sin audio de notificación")
...
def _en_silencio() -> bool:
    ...
    try:
        ...
        inicio = conn.execute("SELECT valor FROM config WHERE clave='silencio_inicio'").fetchone()
...
def _revisar():
    ...
    try:
        ...
        rows_status = conn.execute(
            "SELECT id FROM recordatorios WHERE activo = 1"
        ).fetchall()
        conn.close()
    except Exception:
        rows_status = []
    ...
    if _tray_icon:
        try:
            if rows_status:
                _tray_icon.title = "NeuroMood — Avisos activos"
            else:
                _tray_icon.title = "NeuroMood"
        except Exception:
            pass
    ...
    for row in rows:
        ...
        try:
            with conexion() as conn2:
                conn2.execute("UPDATE recordatorios SET activo = 0 WHERE id = ?", (rec_id,))
        except Exception:
...
def _crear_icono_imagen() -> "PILImage.Image":
    """Devuelve la imagen del ícono. Usa app_icon o NM_icon si existe, sino genera uno simple."""
    try:
        from shared.assets import obtener_ruta_asset, APP_ICON

        icon_path = obtener_ruta_asset(APP_ICON)
    except ImportError:
...
def iniciar_bandeja(stop_event: threading.Event, on_abrir_app=None, on_salir=None):
    ...
    def _salir(icon, item):
        stop_event.set()
        icon.stop()
        # Cerrar la app de verdad. Sin esto el proceso Qt seguía vivo con la
        # ventana oculta y SIN ícono de bandeja → proceso "invisible" que solo
        # se cerraba por el Administrador de tareas.
        if on_salir:
            try:
                on_salir()
            except Exception:
...
def iniciar(on_abrir_app=None, on_salir=None) -> threading.Event:
    ...
    # Bandeja del sistema
    iniciar_bandeja(_stop_event, on_abrir_app=on_abrir_app, on_salir=on_salir)
