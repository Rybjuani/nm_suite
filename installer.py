...
QCheckBox::indicator {{
    ...
    border: 2px solid {BORDER}; background: {BG_SURFACE};
}}
...
class InstaladorNeuroMood(QMainWindow):
    ...
    def _build_p1(self, page: QWidget):
        ...
        for btn, sel in [(self._btn_primera, True), (self._btn_ya, False)]:
            btn.setCheckable(True)
            btn.setFixedHeight(34)
            btn.setChecked(sel)
            btn.setStyleSheet(
                f"QPushButton {{background: {ACCENT if sel else BG_SURFACE};"
                f"color: {TEXT_PRIMARY}; border-radius: 0px; border: none; font-size: 12px;}}"
                f"QPushButton:checked {{background: {ACCENT};}}"
                f"QPushButton:hover {{background: {ACCENT_HOVER};}}"
            )
            seg_row.addWidget(btn)
        self._btn_primera.clicked.connect(lambda: self._cambiar_modo(False))
        self._btn_ya.clicked.connect(lambda: self._cambiar_modo(True))
